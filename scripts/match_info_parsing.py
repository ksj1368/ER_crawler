...
def parse_match_data(data: dict) -> Dict[str, List[Dict]]:
    ...
    try:
        ...
        for u in data.get("userGames", []):
            ...
            # User Start parsing
            user_start_list.append({
                "match_id": match_id, "uid": uid, "nickname": u["nickname"], "character_num": u["characterNum"],
                "language": u.get("language", "None"), "team_number": team_id, "skin_code": u["skinCode"],
            ...
            # Combat info parsing
            user_combat_list.append({
                "match_id": match_id, "uid": uid, "character_level": u["characterLevel"],
            ...
            # Credit Time Series parsing
            for minute in range(20):
                used = u["usedVFCredits"][minute]
                gain = u["totalVFCredits"][minute]
                if used != 0 or gain != 0:
                    user_credit_time_list.append({"match_id": match_id, "uid": uid, "minute": minute, "used_credit": used, "gain_credit": gain})
